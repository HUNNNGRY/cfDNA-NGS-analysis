...
RG_dir = "output/{}/bam-sorted-RG".format(config["dataset"]) if not config["remove_duplications"] else 'output/{}/bam-sorted-deduped-RG'.format(config["dataset"])
BQSR_dir = "output/{}/bam-sorted-RG-BQSR".format(config["dataset"]) if not config["remove_duplications"] else 'output/{}/bam-sorted-deduped-RG-BQSR'.format(config["dataset"])
...
def request(config,outdir,sample_ids):
    ...
    output["vcf"] = expand("{outdir}/vcf-filtered/{sample_id}.vcf.gz",outdir=[outdir],sample_id=sample_ids),
    ...
    output["counts"] = expand("{outdir}/bin-counts/{sample_id}.txt",outdir=[outdir],sample_id=sample_ids)
...
rule bwa_alignment:
    ...
    shell:
        '''
        /BioII/lulab_b/baopengfei/biosoft/bwa-mem2/bwa-mem2 mem -T 0 -t {threads} {params.index} {input.fastq1} {input.fastq2} | samtools view -b > {output.bam} 2> {log}
        '''
...
rule getUnaligned:
    input:
        bam = "{outdir}/bwa_bam/{sample_id}.bam"
    output:
        unmapped_1 = "{outdir}/bwa_unmapped/{sample_id}_1.fastq.gz",
