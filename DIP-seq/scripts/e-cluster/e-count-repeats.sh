...
for r in {LINE,SINE} ; 
do  echo "start $r at `date` " ; 
python3 scripts/summarize-featureCounts.py --indir output/lulab/07counts_allregion/${r} --output output/lulab/matrix/count_matrix_${r}.txt --surfix  _${r};
echo "end $r at `date` "; 
done
