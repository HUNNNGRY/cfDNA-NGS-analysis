...
def request(config,outdir,sample_ids):
    output = dict()
    #output["bam_filter"] = expand("{outdir}/03bam_filter/{sample_id}.bam",outdir=[outdir],sample_id=sample_ids),
    ...
    output["gene_single"] = expand("{outdir}/07counts/{sample_id}_gene",outdir=[outdir],sample_id=sample_ids),
    output["promoter_single"] = expand("{outdir}/07counts/{sample_id}_promoter",outdir=[outdir],sample_id=sample_ids),
    output["CGI_single"] = expand("{outdir}/07counts/{sample_id}_CGI",outdir=[outdir],sample_id=sample_ids),
    output["gene_matrix"]= expand("{outdir}/07counts/count_matrix_gene.txt",outdir=[outdir]),
    output["promoter_matrix"]= expand("{outdir}/07counts/count_matrix_promoter.txt",outdir=[outdir]),
    output["CGI_matrix"]= expand("{outdir}/07counts/count_matrix_CGI.txt",outdir=[outdir])
...
rule multiqc_fastp:
    input:  
        html = expand("{outdir}/01fastp/log/{sample_id}_fastp.html",outdir=[outdir],sample_id=sample_ids),
        json = expand("{outdir}/01fastp/log/{sample_id}_fastp.json",outdir=[outdir],sample_id=sample_ids)
    output:
