...
rule fastp:
    ...
    output:
        fastq1=temp('{outdir}/01fastp/{sample_id}_1.fastq.gz'),
        fastq2=temp('{outdir}/01fastp/{sample_id}_2.fastq.gz')
    params:
...
rule bam_filter:
    ...
    shell:
        """
        (samtools view -H {input.bam}; samtools view -f 2 -F 4 {input.bam} | \
...
rule bam_dedup:
    ...
    output:
        bam = temp("{outdir}/04bam_dedup/{sample_id}.bam"),
        bai = temp("{outdir}/04bam_dedup/{sample_id}.bam.bai"),
...
rule medips_qc:
    ...
    output:
        ...
        coverage = temp("{outdir}/06medips_qc/coverage/{sample_id}.png")
    conda:
        "envs/r35.yml"
    log:
