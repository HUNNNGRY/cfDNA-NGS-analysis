...
#BSUB -J lulab 
#BSUB -q TEST-1U
#BSUB -o /BioII/lulab_b/baopengfei/projects/exOmics/DIP-seq/%J.log
#BSUB -e /BioII/lulab_b/baopengfei/projects/exOmics/DIP-seq/%J.err
#BSUB -n 2
#BSUB -R "span[hosts=1]"
...
dst="test"   # GSE*****, change this when processing other datasets of DIP-seq

# RUN
echo "start at `date`"
echo -e "PATH:""/n""$PATH"
#echo "workdir: /n $workdir"
echo -e "dataset:""/t""$dst"
cd $workdir
snakemake --rerun-incomplete --keep-going --printshellcmds --reason --use-conda --nolock --latency-wait 80 --restart-times 1 --jobs 200  \
	--snakefile scripts/snakefile.py \
	--configfile config/${dst}.yaml \
	--cluster-config config/cluster-lsf.json \
	--cluster "bsub -n {cluster.threads} -J {cluster.jobname} -q {cluster.queue} -o {cluster.output} -e {cluster.error} -R "span[hosts=1]" " \
